...
from pamps.serializers.auth_serializer import RefreshToken, Token
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends(),):
    ...
    access_token = create_access_token(data={"sub": user.username, "fresh": True}, expires_delta=access_token_expires)
...
async def refresh_token(form_data: RefreshToken):
    ...
    access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    access_token = create_access_token(data={"sub": user.username, "fresh": False}, expires_delta=access_token_expires)
    refresh_token_expires = timedelta(minutes=REFRESH_TOKEN_EXPIRE_MINUTES)
    refresh_token = create_refresh_token(data={"sub": user.username}, expires_delta=refresh_token_expires)
